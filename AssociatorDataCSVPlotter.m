...
K = csvread('AssociatorFile.csv');
sz_k = size(K);
num_tracked_obstacles = K(end,1);
max_size = K(end,2);
num_tracked_samples = sz_k(1)-1;
num_tr = [];
num_possible = [];
t = [];
...
for i = 1: num_tracked_obstacles
    ...
    hold on;
    if any(tracked_data(i).time)
        text(tracked_data(i).time(1),tracked_data(i).x(1)-5,num2str(i), 'FontSize',12, 'Color', 'r')
    end
end
xlabel("Time (s)");
ylabel("X co-ordinate (m)");
...
for i = 1: num_tracked_obstacles
    ...
    hold on;
    if any(tracked_data(i).time)
        text(tracked_data(i).time(1),tracked_data(i).y(1)-5,num2str(i), 'FontSize',12, 'Color', 'r')
    end
end
xlabel("Time (s)");
ylabel("Y co-ordinate (m)");
...
for i = 1: num_tracked_obstacles
    ...
    hold on;
    if any(tracked_data(i).time)
        text(tracked_data(i).time(1),tracked_data(i).z(1)-5,num2str(i), 'FontSize',12, 'Color', 'r')
    end
end
xlabel("Time (s)");
ylabel("Z co-ordinate (m)");
